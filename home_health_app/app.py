...
def load_companies(csv_file=CSV_PATH):
    ...
    # Shift emails DOWN by one
    emails = [row.get("email", "").strip() for row in rows]
    shifted_emails = [""] + emails[:-1]

    companies = []
    for row, email in zip(rows, shifted_emails):
        insurances = (
            [i.strip() for i in row["insurance"].split("|")]
            if row["insurance"].strip().lower() != "unknown"
            else []
        )
        service_areas = (
            [a.strip() for a in row["service_area"].split("|")]
            if row["service_area"].strip().lower() != "unknown"
            else []
        )
